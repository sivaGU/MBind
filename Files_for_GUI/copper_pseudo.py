#!/usr/bin/env python
"""Tetrahedral Cu pseudo-atoms (TQ) for AutoDock4 Cu maps — MBind-style."""
import math
...
class PDBQT:
    def __init__(self, line):
        ...
        self.atomnr = self._anr(self.atype)

    def _anr(self, atype):
        D = {"H": 1, "HD": 1, "C": 6, "A": 6, "N": 7, "NA": 7, "OA": 8, "OS": 8, "F": 9,
             "S": 16, "SA": 16, "Cl": 17, "CL": 17, "FE": 26, "CU": 29, "Cu": 29, "ZN": 30,
             "I": 53, "TQ": -1, "TZ": -1}
        u = atype.strip().upper()
    ...
    def isbound(self, other_atom, cut_off_percent=0.5):
        def vdw(n):
            D = {1: 2.0, 6: 4.0, 7: 3.5, 8: 3.2, 9: 3.1, 16: 4.0, 17: 4.1, 26: 1.3, 29: 1.4, 30: 1.5}
            return D.get(n, 2.0)
        th = cut_off_percent * (0.5 * vdw(self.atomnr) + 0.5 * vdw(other_atom.atomnr))
        return self.dist(other_atom) < th

def load_pdbqt(filename):
    atoms_list, max_id, nskip = [], 0, 0
        ...
        for line in f:
            if line.startswith("ATOM  ") or line.startswith("HETATM"):
                ...
                if atom.atype == "TQ":
                    ...
                    continue
                counter += 1
                if canon_ad4_type(atom.atype) == "Cu":
                    atom.charge = 0.0
    ...
    return atoms_list, nskip, max_id, non_atom_text

def bruteNearbyAtoms(atoms_list, metal="Cu", cut_off=4.5):
    mc = canon_ad4_type(metal)
    out = []
    for m in [a for a in atoms_list if canon_ad4_type(a.atype) == mc]:
        idx = sorted(range(len(atoms_list)), key=lambda i: atoms_list[i].dist(m))
        out.append([atoms_list[i] for i in idx if atoms_list[i].dist(m) < cut_off])
    return out
...
def tetra_tq(shell, d=2.0):
    n = len(shell.rec)
    zn = shell.zn.getcoords()
    if n == 3:
        pseudo = deepcopy(shell.zn)
        pseudo.name, pseudo.atype, pseudo.charge = "TQ", "TQ", 0.0
        a, b, c = [x.getcoords() for x in shell.rec]
...
def fallback_tq(metal, rec, d=2.0):
    zn = metal.getcoords()
...
def main():
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            print("python copper_pseudo.py -r receptor.pdbqt [-o out.pdbqt]")
            sys.exit(0)
        if o == "-r":
            inp = a
        if o == "-o":
    ...
    if not inp:
        ...
        sys.exit(2)
    if not outp:
        outp = splitext(inp)[0] + "_TQ.pdbqt"

    r, _, lastserial, nat = load_pdbqt(inp)
    tf_list = []
    for alist in bruteNearbyAtoms(r, "Cu"):
        metal = alist[0]
        sh = MetalShell(metal, 2.5, 0.5)
        sh.proc_rec(alist[1:])
        p = tetra_tq(sh, 2.0) or fallback_tq(metal, sh.rec, 2.0)
        tf_list.append(p)
    for p in tf_list:
        lastserial += 1
        p.serial = lastserial
        r.append(p)
    print(f"Wrote {len(tf_list)} TQ pseudo-atom(s) -> {outp}")
